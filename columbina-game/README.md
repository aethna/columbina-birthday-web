...
- `提瓦特战力党`：横屏骰子对战，双方轮流投掷（掷完可重新投掷）比拼战力，先手由投硬币决定；含 10 关剧情模式、自由对战（自选角色与难度）与三种 AI 难度。
  - 实现上与其他四个不同：它是**零依赖的单文件 HTML**（源码在 `public/teyvat-dice/`，自带素材目录），由 `TeyvatDiceGame.vue` 用 iframe 承载，不进 Vue 打包流程。
